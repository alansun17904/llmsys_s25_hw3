...
def generate(model,
    ...
    for example in tqdm.tqdm(examples, desc=f'Generating {desc}'):
        ...
        while len(token_ids) <= model_max_length:
            ...
            gen_id = 0
            in_toks = minitorch.tensor(token_ids, backend=backend, requires_grad=False)
            in_toks = in_toks.view(1, len(token_ids))
            logits = model(in_toks)
            gen_id = np.argmax(logits.to_numpy()[:,-1,:])
